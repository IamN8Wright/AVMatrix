...
public sealed class MainForm : Form
{
    ...
    private Control BuildSidebar()
    {
        ...
        _logoutButton.Location = new Point(0, 25);
    ...
    private void ChooseLocalMaster()
    {
        ...
        {
            Title = "Choose company .nasc file",
        ...
        try
        {
            ...
            if (!PortableDataService.IsAccountProtected(bytes))
                throw new InvalidDataException(
                    "Choose a company .nasc file generated by InNasc Global Admin. " +
                    "Use Global Admin to migrate older .avmatrix files.");
    ...
    private void SignInSharedMaster(string username, string password)
    {
        ...
        if (string.IsNullOrWhiteSpace(path))
            throw new InvalidOperationException("Choose a company .nasc file first.");
        var bytes = File.ReadAllBytes(path);
        if (!PortableDataService.IsAccountProtected(bytes))
            throw new InvalidDataException(
                "This is not a company .nasc file generated by InNasc Global Admin.");
    ...
    private async Task SignInGoogleMasterAsync(string username, string password)
    {
        ...
        if (!PortableDataService.IsAccountProtected(remote.Contents))
            throw new InvalidDataException(
                "The linked Google Drive file is not a company .nasc generated by InNasc Global Admin.");
        var access = PortableDataService.ReadMasterAccess(remote.Contents);
        var session = MasterAccessService.SignIn(access, username, password);
