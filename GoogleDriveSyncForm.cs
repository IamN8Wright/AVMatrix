...
internal sealed class GoogleDriveSyncForm : Form
{
    ...
    await RunBusyAsync(async () =>
    {
      ...
      if (_legacyDriveSession is not null)
      {
        ...
        var choice = MessageBox.Show(this,
          ...
          "YES  = migrate, then continue only if local and Drive inventories already match\r\n" +
        ...
        {
          var inventoryNotice = string.IsNullOrWhiteSpace(_data.Settings.GoogleDriveFingerprint)
            ? "\r\n\r\nThe local and Drive inventories differ. For safety, InNasc will not overwrite the Drive copy. Pull first; a local recovery copy will be created before the pull."
            : "\r\n\r\nIts existing Drive inventory was left in place. You can Pull or Merge & push normally now.";
          MessageBox.Show(this,
            "The Google Drive company is now protected by InNasc." + inventoryNotice,
  ...
  private async Task<GoogleDriveSnapshot?> InspectWithPasswordAsync(bool prompt)
  {
      ...
      exception is PasswordRequiredException or PasswordProtectionException or MasterLoginRequiredException)
    {
      ...
      if (PortableDataService.IsAccountProtected(raw.Contents))
      {
        if (!prompt)
        {
          _fileState.Text = "Migration required";
          _fileState.ForeColor = UiTheme.Amber;
          _details.Text =
            $"{raw.Metadata.Name}\r\nLegacy AV Matrix account protection detected.";
          _protectionState.Text =
            "Use Pull or Merge & push to sign in to the legacy Drive company and migrate it to InNasc.";
          _protectionState.ForeColor = UiTheme.Amber;
          return null;
        }
    ...
    MasterSession companySession)
  {
    ...
    var localContentFingerprint = SyncContentFingerprint.Compute(_data);
    var remoteContentFingerprint = SyncContentFingerprint.Compute(secured.Contents.Data);
    var inventoriesMatch = string.Equals(
      localContentFingerprint,
      remoteContentFingerprint,
      StringComparison.OrdinalIgnoreCase);
    if (!inventoriesMatch)
    {
      _data.Settings.GoogleDriveFingerprint = string.Empty;
      _data.Settings.GoogleDriveLocalContentFingerprint = localContentFingerprint;
      _data.Settings.GoogleDriveRemoteChangesDetected = true;
      SyncBaselineStore.Delete(_store, SyncTarget.GoogleDrive);
      _store.Save(_data);
    }
    ShowSnapshot(secured);
    _details.Text = inventoriesMatch
      ? $"Legacy AV Matrix Drive protection migrated to InNasc. {migratedPayloads:N0} client payload(s) re-keyed."
      : $"Legacy AV Matrix Drive protection migrated to InNasc. {migratedPayloads:N0} client payload(s) re-keyed. " +
        "The local and Drive inventories differ, so a Pull is required before another push.";
    return secured;
  }
